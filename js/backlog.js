...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('https://script.google.com/macros/s/AKfycbw81EpgMWvtsJ0XkniSWSgyN0PlAM21HIjIriiO2O8yIC1hihL3lkLsSEUoTMa4s7P1Dg/exec')
        .then(response => response.json())
        .then(data => {
            console.log(data);
            window.data = data; // Guarda los datos en una variable global para el filtrado
...
function markAsCompleted(gametitle, platform) {
    ...
    .then(data => {
        console.log(data);
        // Actualiza la tabla local después de mover a completado
        window.data = window.data.filter(item => item.gametitle !== gametitle || item.platform !== platform);
        displayTable(window.data);
    })
    .catch(error => {
        console.error('Error al marcar como completado:', error);
...
function addCompleteButton(row, gametitle, platform) {
    ...
    let button = document.createElement('button');
    button.textContent = 'Finished';
    button.onclick = () => markAsCompleted(gametitle, platform);
    cell.appendChild(button);
}
...
function markAsCompleted(gametitle, platform) {
    ...
    fetch('https://script.google.com/macros/s/AKfycbx9gZqPDHuZZfRc-0t28OYxmmpzxt30gRZrwhg65sGceTbHeR8ukz82eNA0PHWZBeIrmQ/exec', {
        method: 'POST',
        body: formData
    })
    .then(response => response.text())
